...
// Fetch kept dresses that are eligible for pickup
$rental_id = $_GET['rental_id']; // Rental ID passed from the previous page
$stmt = $pdo->prepare("SELECT ri.id, ri.dress_id, d.name AS dress_name, ri.delivery_time, ri.return_time FROM rental_items ri
                        JOIN dresses d ON ri.dress_id = d.id
                        WHERE ri.rental_id = ? AND ri.dress_status = 'may_be_available' AND NOW() >= DATE_ADD(ri.return_time, INTERVAL 1 HOUR)");
$stmt->execute([$rental_id]);
$kept_dresses = $stmt->fetchAll();
...
<head>
    <meta charset="UTF-8">
    <title>Collect Kept Dresses</title>
</head>
<body>
    <h2>Collect Kept Dresses</h2>

    <form action="deliverer_pickup_kept.php" method="POST">
        <ul>
            <?php foreach ($kept_dresses as $dress): ?>
                <li>
                    <input type="checkbox" name="rental_item_ids[]" value="<?= $dress['id'] ?>"> 
                    <?= $dress['dress_name'] ?> (Return Time: <?= $dress['return_time'] ?>)
                </li>
            <?php endforeach; ?>
        </ul>
