...
// Get delivery time (for calculating return time)
$delivery_q = $conn->prepare("SELECT delivery_time FROM rentals WHERE id = ?");
$delivery_q->bind_param("i", $rental_id);
$delivery_q->execute();
$delivery_time = $delivery_q->get_result()->fetch_assoc()['delivery_time'];

// Fetch dresses with their statuses
$query = "SELECT ri.dress_status, d.name, d.image 
...
<body>
        ...
        <div class="dress-container">
            <?php foreach ($kept as $dress): ?>
                <div class="dress-card">
                    ...
                    <p><?= $dress['name'] ?></p>
                    <div class="timer" data-delivery="<?= $delivery_time ?>"></div>
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <script>
        // Add countdown timer for return window
        const timers = document.querySelectorAll('.timer');

        timers.forEach(timer => {
            const deliveryTime = new Date(timer.dataset.delivery);
            const returnTime = new Date(deliveryTime.getTime() + 60 * 60 * 1000); // +1 hour
